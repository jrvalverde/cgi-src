...
$http_tmproot="/data/www/EMBnet/tmp";
...
set_tmp_dir( $random_str, $http_tmproot);
...
$receptor = $_FILES['upload']['name'][0];
$ligand = $_FILES['upload']['name'][1];
...
fwrite( $fp, "# ----------------------------------------------------------------------------\n");
fwrite( $fp, "\n");
fwrite( $fp, "$receptor     *    $receptor  $ligand       *     $ligand\n");
...
echo "following link: <br><br><A HREF=\"http://www.es.embnet.org/tmp/webgramm$random_str\">$random_str</A></CENTER></H2>";
...
//
function set_tmp_dir( $random_str, $http_tmproot )
{   	
	mkdir ("$http_tmproot/webgramm$random_str", 0755);

	copy("./gramm.sh", "$http_tmproot/webgramm$random_str/gramm.sh");
	copy("./Rodin_Penseur.jpg", "$http_tmproot/webgramm$random_str/Rodin_Penseur.jpg");
	copy("./show_results.php", "$http_tmproot/webgramm$random_str/index.php");
	
	chdir( "$http_tmproot/webgramm$random_str" ); 
	`chmod 755 ./gramm.sh`;
}
