...
//Gramm execution control
$control=`more gramm_control`;
chop($control);

//Fixin the values to integers for comparison
settype($control, "integer");
settype($from, "integer");  
settype($to, "integer"); 

echo "HEY, FROM: $from, TO: $to CONTROL $control<br>";
    
if (isset($from) && isset($to))
	//$from and $to are set
	if ( ($from != "") && ($from < $to) && ($from >0) && ($from < 999) && ($to != "")  && ($to > 1)  && ($to <= 1000) )
	    //With correct 'from' and 'to' values we run gramm
	    if ( $from != $control && !file_exists("./started"))
	    {
	    	run_gramm_coord($from, $to);
		//Dirty trick: with the value in gramm_control we manage when gramm should run
		`echo $from > gramm_control`;
	    }
...
if ( file_exists("./done") )
{
	...
	// Code to display the buttons with the correct parameters
	if ( !isset($from) && !isset($to) )
	{
	...
	{   
	    //Validating values
	    if ( ($from == 0) || ($from > $to) || ($from <0) || ($from > 991)|| ($to == 0)  ||($to <0)  || ($to > 1000) )
	    {	//If $from is 0 we are starting the process
	    	echo "<table border=2><tr>";
            	echo "<td valign=\"center\"><p><a href=\"$server$path/index.php?from=11&to=20\"><img SRC=\"./right.gif\" ALT=\"[NEXT]\"></a><td valign=\"center\"> next ten receptor-ligand complexes.</td> ";
		echo "</tr></table>";
	    }elseif ( $from=="1" )
