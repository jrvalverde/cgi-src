<?
// $Id: show_results.php,v 1.1 2003/05/13 13:41:39 root Exp netadmin $
...
if (isset(OLD_CODE)) {	
	// go over the directory contents and generate output listing
	// we want to follow a specific order, therefore we must iterate
	// over the directory listing looking for each file in order.
	$dir->rewind();
	while ($file = $dir->read())
  	{
		// filter out housekeeping files
		if ($file !="." && $file!=".." && $file!="index.php" && $file!="done")
		{
			if ( $file=="receptor.pdb" )
			{
				// show receptor molecule
				echo "<tr>";
				echo "<td>Structure of your receptor molecule.</td>"; 
				echo "<td><a href=./$file>$file</a></td>";
				echo "<td><a href=\"$pdb2vrml1?url=$server$path/$file\">VRML1</a> (<a href=\"$pdb2vrml1?reference\">ref</A>))</td>";
	...
	// Make this into a call to a function show_molecule
		// show receptor molecule
		$file = "receptor.pdb";
		echo "  <tr>\n" .
		     "    <td>Structure of your receptor molecule</td>\n" .
		     "    <td><a href=\"./$file\">$file</a></td>\n" .
		     "    <td><a href=\"$pdb2vrml1?url=$server$path/$file\">VRML1</a> (<a href=\"$pdb2vrml1?reference\">ref</a>)</td>\n" .
		     "    <td><a href=\"$pdb2vrml2?url=$server$path/$file\">VRML2</a></td>\n" .
		     "    <td><a href=\"$pdb2ps?url=$server$path/$file\">PS</a></td>\n" .
		     "  </tr>\n";
	}
}
	// Let's go over it once more to show the ligand
	$dir->rewind();
	while ($file = $dir->read())
  	{
		if ($file !="." && $file!=".." && $file!="index.php" && $file!="done")
	...
	// if we were called with no params, do the default thing
	if ( !isset($from) && !isset($to) )
	{
	    	echo "<table border=2><tr>";
            	echo "<td valign=\"center\"><p><a href=\"$server$path/index.php?from=11&to=20\"><img SRC=\"./right.gif\" ALT=\"[NEXT]\"></a><td valign=\"center\"> next ten receptor-ligand complexes.</td> ";
		echo "</tr></table>";
        }else
	{   
	    // Validate values
	    ...
	    {	
	    	echo "<table border=2><tr>";
            	echo "<td valign=\"center\"><p><a href=\"$server$path/index.php?from=991&to=1000\"><img SRC=\"./left.gif\" ALT=\"[PREVIOUS]\"></a><td valign=\"center\"> previous ten receptor-ligand complexes.</td>";
		echo "</tr></table>";
            }else
		...
		echo "<tr>";
		echo "<td width=50% valign=\"center\"><p><a href=\"$server$path/index.php?from=$from_prev&to=$to_prev\"><center><img SRC=\"./left.gif\" ALT=\"[PREVIOUS]\"></center></a></td>";
		echo "<td width=50% valign=\"center\"><p><a href=\"$server$path/index.php?from=$from_next&to=$to_next\"><center><img SRC=\"./right.gif\" ALT=\"[NEXT]\"></center></a></td>";
		echo "</tr>";
		echo "</table>";
...
{   	
	...
	echo "<td BGCOLOR=\"$color\" VALIGN=\"top\" ALIGN=\"center\"><b> $status </b></td>";
