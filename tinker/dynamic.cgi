...
#
# ------------------------------------------------------------
# dynamic.pl, by José R. Valverde (jrvalverde@es.embnet.org).
#
# Created: September 3, 2001
# Last updated: September 3, 2001
#
# DYNAMIC provides a simplified mechanism to run TINKER DYNAMIC
# over the web.
...
#
# $Id: dynamic.cgi,v 1.3 2002/02/06 12:20:03 netadmin Exp $
# $Log: dynamic.cgi,v $
...
$applongtitle= "Molecular dynamics";
$babel_dir=    "/opt/structure/babel";
$tinker=       "/opt/structure/tinker";
$dynamic=      $tinker . "/bin/dynamic";
...
sub runApp {
	...
	if ( $tinker_pid = fork) {
	    ...
	    # Do molecular dynamics of structure
	    open (IN, ">dynamic.in");
	    print IN "$xyzfile\n$steps\n$stepfs\n";
    	    # For the dumps, we want a minimum: compute total simulation time:
	    $dumpps = ($steps * $stepfs) / 2000;
	    print IN "$dumpps\n$degrees\n";
	    close IN;
	    print JOB "nice $dynamic < dynamic.in >> $logfile 2>&1\n";
	    # save last dump
	    ...
	    print IN "Subject: ### Your Tinker dynamic job $job\n";
	    print IN "\n";
	    close IN;
	    ...
	    if ($havewap) {
	    	...
	    	print JOB "$wapprog -s dynamic $wapmail < wap.msg\n";
    	    }
	    print JOB "cd ..\n";
	    ...
	    print  "<CENTER><BR><STRONG>Running: BABEL + PDBXYZ + DYNAMIC + XYZPDB </STRONG><BR></CENTER>\n";
	    print <<ENDDONE1;
	    &nbsp;<BR>
...
sub mailfile {
    local($file) = @_;
    open (MAIL, "|$mailprog $email") || die "DYNAMIC: Can\'t open $mailprog!\n";
    print MAIL "Reply-to: $email\n";
    print MAIL "Subject: TINKER Molecular Dynamics results\n\n";
