...
#
# ------------------------------------------------------------
# gda.pl, by José R. Valverde (jrvalverde@es.embnet.org).
#
# Created: September 4, 2001
# Last updated: September 4, 2001
#
# GDA provides a simplified mechanism to run TINKER GDA
# over the web.
...
#
# $Id: gda.cgi,v 1.3 2002/02/06 12:20:03 netadmin Exp $
# $Log: gda.cgi,v $
...
$applongtitle= "Gaussian Density Annealing";
...
sub do_work {
	...
	"<H1>ERROR, HORROR: cannot access needed temporary files</H1>\n";
 	...
 	foreach $key ($query->param) {
	  ...
	  elsif ($key =~ /^trials/ && $val) {
	    	$trials = $val;
	    	}
	  elsif ($key =~ /^random/ && $val) {
	    	$random = $val;
	    	}
	  elsif ($key =~ /^ibeta/ && $val) {
	    	$ibeta = $val;
...
sub runApp {
	...
	if ( $tinker_pid = fork) {
	    ...
	    print IN "$xyzfile\n$trials\n$random\n$ibeta, $fbeta\n\n";
	    close IN;
	    print JOB "nice $gda < gda.in >> $logfile 2>&1\n";
	    ...
	    print IN "Subject: ### Your Tinker GDA job $job\n";
	    print IN "\n";
	    close IN;
	    ...
	    if ($havewap) {
	    	open (IN, ">wap.msg");
	    	#print IN "Reply-To: $maintainer\n";
	    	#print IN "Subject: GDA $job\n\n";
	    	print IN "\n\n Your job $job has finished.\n";
	    	close IN;
	    
	    	print JOB "$wapprog -s gda $wapmail < wap.msg\n";
    	    }
	    print JOB "cd ..\n";
	    ...
	    print  "<CENTER><BR><STRONG>Running: BABEL + PDBXYZ + GDA + XYZPDB </STRONG><BR></CENTER>\n";
	    print <<ENDDONE1;
	    &nbsp;<BR>
...
sub mailfile {
    local($file) = @_;
    open (MAIL, "|$mailprog $email") || die "GDA: Can\'t open $mailprog!\n";
    print MAIL "Reply-to: $email\n";
    print MAIL "Subject: TINKER GDA results\n\n";
