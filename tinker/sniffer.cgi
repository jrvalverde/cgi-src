...
#
# ------------------------------------------------------------
# sniffer.pl, by José R. Valverde (jrvalverde@es.embnet.org).
#
# Created: September 4, 2001
# Last updated: September 4, 2001
#
# SNIFFER provides a simplified mechanism to run TINKER SNIFFER
# over the web.
...
#
# $Id: sniffer.cgi,v 1.5 2002/02/06 12:20:03 netadmin Exp $
# $Log: sniffer.cgi,v $
# Revision 1.5  2002/02/06 12:20:03  netadmin
# Fixed mail/wapmail [j]
#
# Revision 1.4  2002/02/06 11:48:19  netadmin
# Minor changes [j]
#
# Revision 1.3  2001/09/05 13:00:55  netadmin
# Re-enabled directory deletion on completion [j]
#
# Revision 1.2  2001/09/05 12:54:41  netadmin
# Fixed for infinite loop due to nsteps [j]
#
# Revision 1.1  2001/09/04 13:37:05  netadmin [jr]
# Initial revision
#
...
$applongtitle= "Molecular Optimization";
$babel_dir=    "/opt/structure/babel";
$tinker=       "/opt/structure/tinker";
$sniffer=      $tinker . "/bin/sniffer";
...
sub runApp {
	...
	if ( $tinker_pid = fork) {
	    ...
	    # minimize structure
	    open (IN, ">sniffer.in");
	    # Commented 'cos anything on $steps produces an infinite loop
	    #print IN "$xyzfile\n$steps\n$emin\n$rms\n\n";
	    print IN "$xyzfile\n\n$emin\n$rms\n\n";
	    close IN;
	    print JOB "nice $sniffer < sniffer.in >> $logfile 2>&1\n";
	    ...
	    print IN "Subject: ### Your Tinker Sniffer job $job\n";
	    print IN "\n";
	    close IN;
	    ...
	    if ($havewap) {
	    	open (IN, ">wap.msg");
	    	#print IN "Reply-To: $maintainer\n";
	    	#print IN "Subject: Sniffer $job\n\n";
	    	print IN "\n\nYour job $job has finished.\n";
	    	close IN;
	    
	    	print JOB "$wapprog -s sniffer $wapmail < wap.msg\n";
    	    }
	    print JOB "cd ..\n";
	    ...
	    print  "<CENTER><BR><STRONG>Running: BABEL + PDBXYZ + SNIFFER + XYZPDB </STRONG><BR></CENTER>\n";
	    print <<ENDDONE1;
	    &nbsp;<BR>
...
sub mailfile {
    local($file) = @_;
    open (MAIL, "|$mailprog $email") || die "SNIFFER: Can\'t open $mailprog!\n";
    print MAIL "Reply-to: $email\n";
    print MAIL "Subject: TINKER SNIFFER results\n\n";
