...
#
# ------------------------------------------------------------
# newton.pl, by José R. Valverde (jrvalverde@es.embnet.org).
#
# Created: September 3, 2001
# Last updated: September 3, 2001
#
# NEWTON provides a simplified mechanism to run TINKER NEWTON
# over the web.
...
#
# $Id: newton.cgi,v 1.3 2002/02/06 12:20:03 netadmin Exp $
# $Log: newton.cgi,v $
...
$applongtitle= "Molecular minimization by Newton methods";
$babel_dir=    "/opt/structure/babel";
$tinker=       "/opt/structure/tinker";
$newton=     $tinker . "/bin/newton";
...
sub do_work {
	...
	"<H1>ERROR, HORROR: cannot access needed temporary files</H1>\n";
 	...
 	foreach $key ($query->param) {
	  ...
	  elsif ($key =~ /^method/ && $val) {
	    	$method = $val;
	    	}
	  elsif ($key =~ /^precond/ && $val) {
	    	$precond = $val;
	    	}
	  elsif ($key =~ /^email/ && $val) {
	    	$email = $val;
...
sub runApp {
	...
	if ( $tinker_pid = fork) {
	    ...
	    print IN "$xyzfile\n$method\n$precond\n$rms\n";
	    close IN;
	    print JOB "nice $newton < newton.in >> $logfile 2>&1\n";
	    
	    # Convert back to PDB
	    open (IN, ">xyzpdb.in");
	    print IN "\n";
	    close IN;
	    # system("$xyzpdb $resfile < xyzpdb.in >> $logfile");
	    print JOB "$xyzpdb $resfile < xyzpdb.in >> $logfile 2>&1\n";

    	    # mail results
	    open (IN, ">mail.header");
	    print IN "Reply-To: $maintainer\n";
	    print IN "Subject: ### Your Tinker Newton job $job\n";
	    print IN "\n";
	    close IN;
	    ...
	    if ($havewap) {
	    	open (IN, ">wap.msg");
	    	#print IN "Reply-To: $maintainer\n";
	    	#print IN "Subject: Newton $job\n\n";
	    	print IN "\n\nYour job $job has finished.\n";
	    	close IN;
	    
	    	print JOB "$wapprog -s newton $wapmail < wap.msg\n";
    	    }
	    print JOB "cd ..\n";
	    ...
	    print  "<CENTER><BR><STRONG>Running: BABEL + PDBXYZ + NEWTON + XYZPDB </STRONG><BR></CENTER>\n";
	    print <<ENDDONE1;
	    &nbsp;<BR>
...
sub mailfile {
    local($file) = @_;
    open (MAIL, "|$mailprog $email") || die "NEWTON: Can\'t open $mailprog!\n";
    print MAIL "Reply-to: $email\n";
    print MAIL "Subject: TINKER Newton results\n\n";
