...
//
// $Id: pdb2vrml2-ws.php,v 1.1 2005/01/24 09:56:42 netadmin Exp $
// $Log: pdb2vrml2-ws.php,v $
...
$server->configureWSDL('pdb2vrml2', 'urn:pdb2vmlr2');
...
$server->register('source_code',    	    	    // method name
	...
	'urn:pdb2vrml2',	    	    	    	    // namespace
	'urn:pdb2vrml2#source_code',    	    	    // soapaction
...
$server->register('VRML2_virtual_world_URL',  // method name
    array('pdb_url' => 'xsd:string'),         // input parameters
    array('return' => 'xsd:string'),          // output parameters
    'urn:pdb2vrml2',                          // namespace
    'urn:pdb2vrml2#VRML2_virtual_world_URL',  // soapaction
    'rpc',                                    // style
    'encoded',                                // use
    'Generate a VRML-2 virtual world containing the PDB structure provided'            // documentation
);

$server->register('VRML2_virtual_world',  // method name
    array('pdb_data' => 'xsd:string'),    // input parameters
    array('return' => 'xsd:string'),      // output parameters
    'urn:pdb2vrml2',                      // namespace
    'urn:pdb2vrml2#VRML2_virtual_world',  // soapaction
    'rpc',                                // style
    'encoded',                            // use
    'Generate a VRML-2 virtual world containing the PDB structure provided'            // documentation
);
...
if(isset($log) and $log != ''){
	harness('pdb2vrml2',$server->headers['User-Agent'],$server->methodname,$server->request,$server->response,$server->result);
}
...
function source_code() {
	$source = file_get_contents("./pdb2vrml2-ws.php");
	return $source;
}
...
function usage()
{
    return "<CENTER><H1>pdb2vrml2</H1></CENTER>" .
    "<P><STRONG>PDB2VRML2</STRONG>: convert a PDB file to a VRML 2 virtual world</P>" .
    "<P><STRONG>Usage:</STRONG><PRE>\n" .
    "string pdb2vrml2->source_code()     // return source code of this server\n" .
    "string pdb2vrml2->usage()           // return usage instructions (this message)\n" .
    "string pdb2vrml2->bibliography()    // return bibliographic references\n" .
    "string pdb2vrml2->VRML2_virtual_world(string) // convert PDB data to VRML2 virtual world\n" .
    "string pdb2vrml2->VRML2_virtual_world_URL(string) // retrieve PDB data from URL and convert to VRML2 virtual world\n" .
    "</PRE>";
}

function VRML2_virtual_world_URL($pdb_url)
{
...
function VRML2_virtual_world($pdb)
{
    global $system_tmproot, $debug;
...
function generate_vrml2()
{
    ...
    if ( !file_exists("./structure.wrl") || 
    	(($size_wrl=filesize("./structure.wrl"))==0) )
    {
	return false;
    } else
    {
