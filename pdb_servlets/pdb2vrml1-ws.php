...
//
// $Id: pdb2vrml1-ws.php,v 1.2 2005/01/21 15:35:54 netadmin Exp $
// $Log: pdb2vrml1-ws.php,v $
...
$server->configureWSDL('pdb2vrml1', 'urn:pdb2vmlr1');
...
$server->register('source_code',    	    	    // method name
	...
	'urn:pdb2vrml1',	    	    	    	    // namespace
	'urn:pdb2vrml1#source_code',    	    	    // soapaction
...
$server->register('VRML1_virtual_world_URL',  // method name
    array('pdb_url' => 'xsd:string'),         // input parameters
    array('return' => 'xsd:string'),          // output parameters
    'urn:pdb2vrml1',                          // namespace
    'urn:pdb2vrml1#VRML1_virtual_world_URL',  // soapaction
    'rpc',                                    // style
    'encoded',                                // use
    'Generate a VRML-1 virtual world containing the PDB structure provided'            // documentation
);

$server->register('VRML1_virtual_world',  // method name
    array('pdb_data' => 'xsd:string'),    // input parameters
    array('return' => 'xsd:string'),      // output parameters
    'urn:pdb2vrml1',                      // namespace
    'urn:pdb2vrml1#VRML1_virtual_world',  // soapaction
    'rpc',                                // style
    'encoded',                            // use
    'Generate a VRML-1 virtual world containing the PDB structure provided'            // documentation
);
...
if(isset($log) and $log != ''){
	harness('pdb2vrml1',$server->headers['User-Agent'],$server->methodname,$server->request,$server->response,$server->result);
}
...
function source_code() {
	$source = file_get_contents("./pdb2vrml1-ws.php");
	return $source;
}
...
{
	$biblio = array(
		'<A HREF=\"http://www.pc.chemie.tu-darmstadt.de/research/vrml/pdb2vrml.html\">http://www.pc.chemie.tu-darmstadt.de/research/vrml/pdb2vrml.html</A>');
	return $biblio;
}
...
function usage()
{
    return "<CENTER><H1>pdb2vrml1</H1></CENTER>" .
    "<P><STRONG>PDB2VRML1</STRONG>: convert a PDB file to a VRML 1 virtual world</P>" .
    "<P><STRONG>Usage:</STRONG><PRE>\n" .
    "string pdb2vrml1->source_code()     // return source code of this server\n" .
    "string pdb2vrml1->usage()           // return usage instructions (this message)\n" .
    "string pdb2vrml1->bibliography()    // return bibliographic references\n" .
    "string pdb2vrml1->VRML1_virtual_world(string) // convert PDB data to VRML1 virtual world\n" .
    "string pdb2vrml1->VRML1_virtual_world_URL(string) // retrieve PDB data from URL and convert to VRML1 virtual world\n" .
    "</PRE>";
}

function VRML1_virtual_world_URL($pdb_url)
{
    global $system_tmproot, $debug, $pdb2vrml;
    ...
    if ( generate_vrml1() ) 
    {
    	...
    	$world = file_get_contents("./structure.vrml");
...
function VRML1_virtual_world($pdb)
{
    global $system_tmproot, $debug, $pdb2vrml;
