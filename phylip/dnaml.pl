...
# ------------------------------------------------------------
# DNAPars.pl, by José R. Valverde (jrvalverde@es.embnet.org).
#
# Created: January 13, 2000
# Last updated: January 13, 2000
#
# DNAPars provides a mechanism by which users of a World-
...
$dnaparsprog = '/opt/evolution/phylip/bin/dnaml';
...
print "<H1>Thank you for using DNAML</H1>\n<BR>\n";
...
#   1. mkdir /tmp/tmpdir$$
srand;
$tmpdir = sprintf "%s/%s.%d.%d", $systmp, "xdnaml", $$, int(rand(30000));
...
#print  "<P>Running DNAML...";
open (DNAML, "|$dnaparsprog >$tmpdir/dmoutput") || die "<H1>Can't run DNAML</H1>\n";
#open (DNAML, "|$dnaparsprog") || die "<H1>Can't run DNAPARS</H1>\n";
...
if ($FORM{'or'}) {
    ...
    if ($outgroup != 0) {
    	if ($outgroup > $noseqs) {
	    print "<P>WARING: Outgroup number is greater than the number of species</P>\n";
	    printf "<P>(%d > %d) NOT using outgroup</P>\n", $outgroup, $noseqs;
    	}
	else {
	printf DNAML "O\n%s\n", $outgroup;
    	    printf "<P>Using sequence number %d as ougroup</P>", $outgroup;
	}
    }
}
...
if ($FORM{'tt'}) {
    printf DNAML "T\n%s\n", $FORM{'tt'};
    printf "<P>Using transition/transversion ratio of %s</P>\n", $FORM{'tt'};
}
else {
    print "<P>Using transition/transversion ratio of 2.0</P>\n";
}
#optionM -> we don't do multiple data sets
#optionI -> we have converted to interleaved format
#option0 -> we are always ANSI
print DNAML "G\n" if  $FORM{'gre'} =~ /y/i;	# Global rearrangements?
print DNAML "1\n";	    	    	    	# print out data at start of run
print DNAML "2\n";	    	    	    	# do not print indications of progress
print DNAML "3\n" if  $FORM{'pot'} =~ /n/i;	# Print out tree?
print DNAML "5\n" if  $FORM{'rhs'} =~ /y/i;     # Reconstruct hypothetical sequences?

print DNAML "Y\n";

#Note: this is not standard behaviour XXX - CNB - XXX
#print DNAPARS "treefile\n";
...
close (DNAML);   # wait for DNAML to finish
# Now $? contains dnaml exit status
#print " done</P>\n";
...
# Remove all files
unlink "$tmpdir/infile", "$tmpdir/dmoutput", "$tmpdir/outfile", "$tmpdir/outtree";
chdir "..";
