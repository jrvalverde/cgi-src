...
$dnaparsprog = '/opt/evolution/phylip/bin/dnapars';
...
print "<H1>Thank you for using DNAPARS</H1>\n<BR>\n";
...
#   1. mkdir /tmp/tmpdir$$
srand;
$tmpdir = sprintf "%s/%s.%d.%d", $systmp, "xdnapars", $$, int(rand(30000));
...
#print  "<P>Running DNAPARS...";
open (DNAPARS, "|$dnaparsprog >$tmpdir/dpoutput") || die "<H1>Can't run DNAPARS</H1>\n";
#open (DNAPARS, "|$dnaparsprog") || die "<H1>Can't run DNAPARS</H1>\n";

#Note: this is not standard behaviour XXX - CNB - XXX
#print DNAPARS "infile\n";
#Note: this is not standard behaviour XXX - CNB - XXX
#print DNAPARS "outfile\n";
...
close (DNAPARS);   # wait for DNAPARS to finish
...
# Remove all files
unlink "$tmpdir/infile", "$tmpdir/dpoutput", "$tmpdir/outfile", "$tmpdir/outtree";
chdir "..";
